...
/////////////////////////////////////////////////////////////////////////////////////////
$site_name="ZOIKS!";
$dm_version=file_get_contents("version")." build(".file_get_contents("build.dat").")";
...
function thumb($im) {
    $thumb=ImageCreate(64,48); // copy original image to thumbnail
    ImageCopyResized($thumb,$im,0,0,0,0,64,48,ImageSX($im),ImageSY($im)); // show thumbnail on screen
    $out = ImagejpeG($thumb);
    print($out);
}
...
function dm_kill($what) {
    echo "<html><head><title>$site_name</title></head>\n";
