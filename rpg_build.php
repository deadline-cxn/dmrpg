...
if($data->access<255) {
    ...
    if($action=="mappaint") {        // Paint mode
        ...
        if(empty($res)) {
            dm_query("insert into rpg_map (`location`, `image`) values('$loc', '$mapbrush')");
        }
        else
    ...
    if($action=="maped") {
        ...
        while(list ($key, $file) = each ($dirfiles)) {
            ...
            $file<br>[<a href=rpg_build.php?action=mapedmmi&image=$file&loc=$loc>use image</a>]<br>
    ...
    if($action=="monsteredgo") {
        ...
        $name=addslashes($name);
        dm_query("update rpg_monsters set `name`='$name' where id='$id'");
        dm_query("update rpg_monsters set `image`='$image' where id='$id'");
        $loot_table=$_REQUEST['loot_table'];
        dm_query("update rpg_monsters set `loot_table`='$loot_table' where id='$id'");
        $level_low=$_REQUEST['level_low'];
        dm_query("update rpg_monsters set `level_low`='$level_low' where id='$id'");
        $level_high=$_REQUEST['level_high'];
        dm_query("update rpg_monsters set `level_high`='$level_high' where id='$id'");
        $str=$_REQUEST['str'];
        dm_query("update rpg_monsters set `str`='$str' where id='$id'");
        $int=$_REQUEST['int'];
        dm_query("update rpg_monsters set `int`='$int' where id='$id'");
        $agl=$_REQUEST['agl'];
        dm_query("update rpg_monsters set `agl`='$agl' where id='$id'");
        $def=$_REQUEST['def'];
        dm_query("update rpg_monsters set `def`='$def' where id='$id'");
        $hp=$_REQUEST['hp'];
        dm_query("update rpg_monsters set `hp`='$hp' where id='$id'");
        $hp_max=$_REQUEST['hp_max'];
        dm_query("update rpg_monsters set `hp_max`='$hp_max' where id='$id'");	
        $pow=$_REQUEST['pow'];
        dm_query("update rpg_monsters set `pow`='$pow' where id='$id'");
        $pow_max=$_REQUEST['pow_max'];
        dm_query("update rpg_monsters set `pow_max`='$pow_max' where id='$id'");
        $dmg_low=$_REQUEST['dmg_low'];
        dm_query("update rpg_monsters set `dmg_low`='$dmg_low' where id='$id'");
        $dmg_high=$_REQUEST['dmg_high'];
        dm_query("update rpg_monsters set `dmg_high`='$dmg_high' where id='$id'");
        $alignment=$_REQUEST['alignment'];
        dm_query("update rpg_monsters set `alignment`='$alignment' where id='$id'");
        $group=$_REQUEST['group'];
        dm_query("update rpg_monsters set `group`='$group' where id='$id'");
        $action="monster";
    }
